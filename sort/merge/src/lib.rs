...
/// Basic O(nlog(n)) sorting algorithm. This should be as few allocations as possible
/// while not mutating the input slice.
pub fn mergesort<T: Ord + Copy + Debug>(items: &[T]) -> Vec<T> {
    ...
    let sorted_front = mergesort(&items[0..split_point]);
    let sorted_back = mergesort(&items[split_point..]);
